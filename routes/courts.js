    ...
    if (req.params.id) {
        if (req.user.type == 'owner') {
            if (req.body.name && req.body.players && req.body.hourprice && req.body.address && req.body.neighborhood && req.body.description && req.body.services && req.body.calendar) {   
                ...
                await data.update(parseInt(req.user._id), parseInt(req.params.id), court)
                    .then((result) => {
                        if (result != null) {
                            res.json(result);
                        } else {
                            res.status(403).send({"description": "you can not update that court"});
                        }                        
                    })
                    .catch((err) => {
                        res.status(500).send({"description": "something went wrong, err: " + err});
...
router.delete('/:id', authenticateToken, async (req, res)=>{
    if (req.params.id) {
        if (req.user.type == 'owner') {
            await data.deleteCourt(parseInt(req.params.id), parseInt(req.user._id))
                    .then((result) => {
                        if (result != null) {
                            res.json(result);
                        } else {
                            res.status(403).send({"description": "you can not delete that court"});
                        }                        
                    })
                    .catch((err) => {
                        res.status(500).send({"description": "something went wrong, err: " + err});
